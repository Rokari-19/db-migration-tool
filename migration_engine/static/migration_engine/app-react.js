(() => {
  ...
  const notify = (message, type = 'error') => window.dispatchEvent(new CustomEvent('app:notify', { detail: { message, type } }));

  async function api(url, opts = {}) {
    const headers = { 'Content-Type': 'application/json', ...(opts.headers || {}) };
    const token = getToken();
    if (token) headers.Authorization = token.includes('.') ? `Bearer ${token}` : `Token ${token}`;
    ...
    if (!response.ok) {
      ...
      const msg = `Request failed (${response.status}): ${body || response.statusText}`;
      notify(msg, 'error');
      throw new Error(msg);
    }
  ...
  function Layout({ children }) {
    ...
    useEffect(() => {
      const onNotify = (e) => { setAlert(e.detail); setTimeout(() => setAlert(null), 4500); };
    ...
    return h('div', null,
      alert && h('div', { className: `alert ${alert.type}` }, alert.message),
  ...
  function Hero() { return h(Layout, null, h('section', { className: 'hero card' }, h('h1', null, 'Simple, Safe Database Migrations'), h('p', null, 'Designed for non-technical teams.'), h('div', { className: 'hero-actions' }, h('a', { href: '/migrations/new/', role: 'button' }, 'Start a Migration'), h('a', { href: '/dashboard/', role: 'button', className: 'secondary' }, 'View Dashboard')))); }
  ...
  function Login() {
    ...
    const submit = async (e) => {
      ...
      try {
        const data = await api('/dj-rest-auth/login/', { method: 'POST', body: JSON.stringify({ username: fd.get('username'), password: fd.get('password') }) });
    ...
    return h(Layout, null, h('article', { className: 'card' }, h('h2', null, 'Sign in'), h('form', { onSubmit: submit }, h('input', { name: 'username', placeholder: 'Username', required: true }), h('input', { name: 'password', type: 'password', placeholder: 'Password', required: true }), h('button', { type: 'submit' }, 'Login')), h('p', { className: 'helper' }, msg)));
  }

  function Dashboard() { const [jobs, setJobs] = useState([]); useEffect(() => { api(`${apiBase}/jobs`).then(setJobs).catch(() => {}); }, []);
    return h(Layout, null, h('h1', null, 'Dashboard'), h('a', { href: '/migrations/new/', role: 'button' }, 'Start New Migration'), h('table', null, h('thead', null, h('tr', null, h('th', null, 'Name'), h('th', null, 'Route'), h('th', null, 'Status'), h('th', null, 'Updated'))), h('tbody', null, jobs.length ? jobs.map((j) => h('tr', { key: j.id }, h('td', null, h('a', { href: `/migrations/${j.id}/` }, j.name)), h('td', null, `${j.source_profile_name} → ${j.target_profile_name}`), h('td', null, j.status), h('td', null, new Date(j.updated_at).toLocaleString()))) : h('tr', null, h('td', { colSpan: 4 }, 'No jobs yet.'))))); }

  function ConnectionPanel({ side, profiles, state, setState, onProfileCreated }) {
    ...
    return h('fieldset', { className: 'card' }, h('legend', null, side),
      ...
      mode === 'saved' ? h('select', { value: state.profile_id || '', onChange: (e) => assignFromProfile(e.target.value) }, [h('option', { value: '', key: 'blank' }, 'Select profile')].concat(profiles.map((p) => h('option', { key: p.id, value: p.id }, `${p.name} (${p.db_type})`)))) : h('div', null,
        ...
        h('button', { className: 'secondary', type: 'button', onClick: async () => {
          ...
          notify(`Profile ${created.name} created.`, 'ok');
          onProfileCreated(created, side.toLowerCase());
        } }, 'Save Profile')
  ...
  function Wizard() {
    ...
    useEffect(() => { api(`${apiBase}/profiles`).then(setProfiles).catch(() => {}); }, []);
    const onProfileCreated = (created, side) => { setProfiles((p) => [...p, created]); if (side === 'source') setSource((s) => ({ ...s, mode: 'saved', profile_id: created.id })); else setTarget((s) => ({ ...s, mode: 'saved', profile_id: created.id })); };
    ...
    return h(Layout, null, h('h1', null, 'Guided Migration Wizard'), h('progress', { value: step, max: 4 }),
      step === 1 && h('div', null, h('p', { className: 'helper' }, 'Pick saved profiles or create new connections.'), h('div', { className: 'form-grid' }, h(ConnectionPanel, { side: 'Source', profiles, state: source, setState: setSource, onProfileCreated }), h(ConnectionPanel, { side: 'Target', profiles, state: target, setState: setTarget, onProfileCreated })), h('button', { onClick: testConnections }, 'Test Connections'), h('p', null, msg)),
      step === 2 && h('form', { onSubmit: async (e) => { e.preventDefault(); const fd = new FormData(e.target); const p = await api(`${apiBase}/plan`, { method: 'POST', body: JSON.stringify({ name: fd.get('name'), source_profile_id: Number(source.profile_id), target_profile_id: Number(target.profile_id), old_table: fd.get('old_table'), new_table: fd.get('new_table'), column_mapping: JSON.parse(fd.get('column_mapping') || '{}'), batch_size: Number(fd.get('batch_size') || 500), stop_on_error: !!fd.get('stop_on_error') }) }); setPlan(p); setStep(3);} }, h('input', { name: 'name', placeholder: 'Migration name', required: true }), h('input', { name: 'old_table', placeholder: 'Source table', required: true }), h('input', { name: 'new_table', placeholder: 'Target table', required: true }), h('textarea', { name: 'column_mapping', defaultValue: '{"id":"id"}' }), h('input', { name: 'batch_size', type: 'number', defaultValue: 500 }), h('label', null, h('input', { type: 'checkbox', name: 'stop_on_error', defaultChecked: true }), ' Stop on error'), h('button', { type: 'submit' }, 'Create Plan')),
  ...
  function Job({ jobId }) { const [status, setStatus] = useState(null); const [logs, setLogs] = useState([]);
    useEffect(() => { let t; const run = async () => { const [s,l] = await Promise.all([api(`${apiBase}/${jobId}/status`), api(`${apiBase}/${jobId}/logs`)]); setStatus(s); setLogs(l.results || []); if (!['SUCCESS','FAILED'].includes(s.status)) t = setTimeout(run, 2000); }; run().catch(()=>{}); return () => clearTimeout(t); }, [jobId]);
